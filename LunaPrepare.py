...
def generate_spacing_mhd():
    ...
    df = pd.DataFrame({'seriesuid': uids,
                       'spacingX': [i[0] for i in spacing], 'spacingY': [i[1] for i in spacing], 'spacingZ': [i[2] for i in spacing],
                       'originX': [i[0] for i in origin], 'originY': [i[1] for i in origin], 'originZ': [i[2] for i in origin],
                       'sizeX': [i[0] for i in size], 'sizeY': [i[1] for i in size], 'sizeZ': [i[2] for i in size]}
                        )

    df.to_csv(f'{config.csv_path}/MhdOriginAndSpacing.csv')
